        ...
        <li className="flex items-center justify-between p-2 space-x-2 cursor-pointer hover:bg-gray-100 group">
            <label className="flex items-center cursor-pointer relative">
                <input
                    ...
                    className="peer h-5 w-5 cursor-pointer transition-all appearance-none rounded-full bg-slate-100 shadow hover:shadow-md border border-slate-300 checked:bg-slate-800 checked:border-slate-800"
                />
            ...
            {isEditing ? (
                <form onSubmit={handleUpdate} className="flex-grow">
                    <input
                        ...
                        className={`w-full border-0 text-lg focus:outline-none ${isEditing ? 'border-b border-slate-900' : ''}`}
                    />
                </form>
            ) : (
                <span
